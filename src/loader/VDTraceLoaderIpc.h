...
#ifndef VDTRACE_LOADER_PIPE_NAME
#define VDTRACE_LOADER_PIPE_NAME L"\\\\.\\pipe\\VDTraceLoaderControl"
#endif

#ifndef VDTRACE_LOADER_MAX_PATH_CHARS
#define VDTRACE_LOADER_MAX_PATH_CHARS 1024
#endif

#ifndef VDTRACE_LOADER_MAX_TEXT_CHARS
#define VDTRACE_LOADER_MAX_TEXT_CHARS 256
#endif
...
namespace VDTraceLoaderIpc
{
    inline constexpr std::uint32_t kMagic = 0x5654444C;
    inline constexpr std::uint32_t kProtocolVersion = 1;
    inline constexpr std::uint32_t kFeatureBootstrapAfterLoad = 0x00000001;
    ...
    {
        wchar_t processPath[VDTRACE_LOADER_MAX_PATH_CHARS];
    ...
    {
        char text[VDTRACE_LOADER_MAX_TEXT_CHARS];
    };

    struct LoadDllRequestPayload
    {
        wchar_t dllPath[VDTRACE_LOADER_MAX_PATH_CHARS];
    };
    ...
    struct LoadDllReplyPayload
    {
        ...
        wchar_t dllPath[VDTRACE_LOADER_MAX_PATH_CHARS];
        wchar_t text[VDTRACE_LOADER_MAX_TEXT_CHARS];
    };
    ...
    inline bool WriteAll(HANDLE pipeHandle, const void *buffer, DWORD size)
    {
        ...
        while (totalWritten < size)
        {
            ...
            if (!WriteFile(pipeHandle, bytes + totalWritten, size - totalWritten, &bytesWritten, nullptr) || bytesWritten == 0)
            {
                return false;
            }
    ...
    inline bool ReadAll(HANDLE pipeHandle, void *buffer, DWORD size)
    {
        ...
        {
            DWORD bytesRead = 0;
            if (!ReadFile(pipeHandle, bytes + totalRead, size - totalRead, &bytesRead, nullptr) || bytesRead == 0)
            {
                return false;
            }
            totalRead += bytesRead;
    ...
    inline bool WriteMessage(HANDLE pipeHandle, MessageKind kind, std::uint32_t pid, const void *payload, std::uint32_t payloadSize)
    {
    ...
    inline bool ReadMessage(HANDLE pipeHandle, std::vector<std::uint8_t> &buffer)
    {
        ...
        return payloadSize == 0 || ReadAll(pipeHandle, buffer.data() + sizeof(header), payloadSize);
    }

    inline HANDLE ConnectToPipe(DWORD timeoutMs)
    {
        if (!WaitNamedPipeW(VDTRACE_LOADER_PIPE_NAME, timeoutMs))
        {
            return INVALID_HANDLE_VALUE;
        }

        return CreateFileW(VDTRACE_LOADER_PIPE_NAME, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    }
    ...
    inline bool SendAgentHello(HANDLE pipeHandle, std::uint32_t pid, const std::wstring &processPath)
    {
        ...
        return WriteMessage(pipeHandle, MessageKind::AgentHello, pid, &payload, static_cast<std::uint32_t>(sizeof(payload)));
    }
    ...
    inline bool SendAgentLog(HANDLE pipeHandle, std::uint32_t pid, const char *text)
    {
        ...
        return WriteMessage(pipeHandle, MessageKind::AgentLog, pid, &payload, static_cast<std::uint32_t>(sizeof(payload)));
    }
    ...
    inline bool SendLoadDllRequest(HANDLE pipeHandle, std::uint32_t pid, const std::wstring &dllPath)
    {
        ...
        return WriteMessage(pipeHandle, MessageKind::LoadDllRequest, pid, &payload, static_cast<std::uint32_t>(sizeof(payload)));
    }

    inline bool SendLoadDllReply(HANDLE pipeHandle, std::uint32_t pid, std::uint32_t status, std::uint32_t win32Error, const std::wstring &dllPath, const std::wstring &text)
    {
        ...
        return WriteMessage(pipeHandle, MessageKind::LoadDllReply, pid, &payload, static_cast<std::uint32_t>(sizeof(payload)));
    }
}
