...
namespace
{
    ...
    {
        std::printf("Usage:\n");
        std::printf("  vdtrace_memory_client.exe read <pid> <address> <size>\n");
        std::printf("  vdtrace_memory_client.exe write <pid> <address> <hex bytes...>\n");
        std::printf("Examples:\n");
        std::printf("  vdtrace_memory_client.exe read 1234 0x140000000 64\n");
        std::printf("  vdtrace_memory_client.exe write 1234 0x140000000 90 90 CC\n");
    }
    ...
    {
        using Status = VDTraceLoaderMemoryIpc::OperationStatus;
    ...
    void PrintReply(const VDTraceLoaderMemoryIpc::MemoryReplyPayload& reply)
    {
    ...
    bool ReceiveReply(
        HANDLE pipeHandle,
        VDTraceLoaderMemoryIpc::MessageKind expectedKind,
        VDTraceLoaderMemoryIpc::MemoryReplyPayload& reply)
    {
        ...
        if (buffer.size() < sizeof(VDTraceLoaderMemoryIpc::MessageHeader) + sizeof(reply))
        {
            return false;
        }
        ...
        const auto* payload = reinterpret_cast<const VDTraceLoaderMemoryIpc::MemoryReplyPayload*>(
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    HANDLE pipeHandle = VDTraceLoaderMemoryIpc::ConnectToPipe(pid, 1000);
    ...
    if (_wcsicmp(command.c_str(), L"read") == 0)
    {
        ...
        if (!VDTraceLoaderMemoryIpc::SendMemoryReadRequest(
                pipeHandle,
                GetCurrentProcessId(),
                requestId,
                address,
    ...
    else if (_wcsicmp(command.c_str(), L"write") == 0)
    {
        ...
        if (!VDTraceLoaderMemoryIpc::SendMemoryWriteRequest(
                pipeHandle,
                GetCurrentProcessId(),
                requestId,
                address,
    ...
    PrintReply(reply);
    if (reply.status != static_cast<std::uint32_t>(VDTraceLoaderMemoryIpc::OperationStatus::Success))
    {
        exitCode = 10;
    }
