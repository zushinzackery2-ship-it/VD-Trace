...
namespace VDTraceLoader
{
    ...
    namespace MemoryRuntime
    {
        DWORD WINAPI MemoryIpcThreadProc(void*)
        {
            ...
            AppendRuntimeLog("Memory IPC pipe = " + Narrow(VDTraceLoaderMemoryIpc::BuildPipeName(GetCurrentProcessId())));
            ...
            {
                HANDLE pipeHandle = VDTraceLoaderMemoryIpc::CreatePipeServerInstance(GetCurrentProcessId());
                ...
                if (!VDTraceLoaderMemoryIpc::WaitForPipeClient(pipeHandle))
                {
                    AppendRuntimeLog("memory-ipc wait client failed error = " + std::to_string(GetLastError()));
                    VDTraceLoaderMemoryIpc::ClosePipe(pipeHandle);
    ...
    {
        if (!IsAllowedControllerProcess() || gMemoryRuntimeThread != nullptr)
        {
            return;
        }
