...
namespace VDTraceLoader::MemoryRuntime
{
    ...
    thread_local VDTraceLoaderMemoryIpc::MemoryReplyPayload* gMemoryAccessReply = nullptr;
    ...
    {
        std::lock_guard<std::mutex> lock(gMemoryRegionCacheMutex);
        gMemoryRegionCache.erase(
            std::remove_if(gMemoryRegionCache.begin(), gMemoryRegionCache.end(),
                [address](const MemoryRegionCacheEntry& entry)
                {
                    return address >= entry.baseAddress && address < entry.endAddress;
                }),
            gMemoryRegionCache.end());
    }
        ...
        VDTraceLoaderMemoryIpc::MemoryReplyPayload& reply)
    {
        ...
        while (remaining != 0)
        {
            ...
            const auto allowed = accessKind == VDTraceLoaderMemoryIpc::AccessKind::Read
            ...
            {
                reply.status = static_cast<std::uint32_t>(VDTraceLoaderMemoryIpc::OperationStatus::ProtectionDenied);
    ...
    LONG CALLBACK MemoryAccessVectoredHandler(EXCEPTION_POINTERS* exceptionPointers)
    {
        ...
        reply.status = static_cast<std::uint32_t>(VDTraceLoaderMemoryIpc::OperationStatus::ExceptionCaught);
        ...
        VDTraceLoaderMemoryIpc::MemoryReplyPayload& reply)
    {
        ...
        gMemoryAccessContext.accessKind = VDTraceLoaderMemoryIpc::AccessKind::Read;
        ...
        VDTraceLoaderMemoryIpc::MemoryReplyPayload& reply)
    {
        bool success = false;
        gMemoryAccessContext.active = true;
        gMemoryAccessContext.accessKind = VDTraceLoaderMemoryIpc::AccessKind::Write;
        gMemoryAccessContext.requestId = reply.requestId;
        gMemoryAccessContext.address = address;
        gMemoryAccessContext.size = size;
        gMemoryAccessReply = &reply;
