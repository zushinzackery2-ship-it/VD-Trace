...
#ifndef VDTRACE_LOADER_MEMORY_PIPE_PREFIX
#define VDTRACE_LOADER_MEMORY_PIPE_PREFIX L"\\\\.\\pipe\\VDTraceLoaderMemory-"
#endif

#ifndef VDTRACE_LOADER_MAX_MEMORY_BYTES
#define VDTRACE_LOADER_MAX_MEMORY_BYTES 16384
#endif

#ifndef VDTRACE_LOADER_SHARED_MEMORY_BYTES
#define VDTRACE_LOADER_SHARED_MEMORY_BYTES 1048576
#endif

#ifndef VDTRACE_LOADER_SHARED_NAME_CHARS
#define VDTRACE_LOADER_SHARED_NAME_CHARS 128
#endif
...
namespace VDTraceLoaderMemoryIpc
{
    ...
    struct MemoryWriteRequestPayload
    {
        ...
        std::uint32_t size;
        std::uint32_t reserved;
        std::uint8_t data[VDTRACE_LOADER_MAX_MEMORY_BYTES];
    };
    ...
    struct SharedMemoryConnectRequestPayload
    {
        ...
        wchar_t mappingName[VDTRACE_LOADER_SHARED_NAME_CHARS];
        wchar_t requestEventName[VDTRACE_LOADER_SHARED_NAME_CHARS];
        wchar_t replyEventName[VDTRACE_LOADER_SHARED_NAME_CHARS];
    };
    ...
    struct SharedMemoryBlock
    {
        ...
        std::uint8_t requestData[VDTRACE_LOADER_SHARED_MEMORY_BYTES];
        std::uint8_t replyData[VDTRACE_LOADER_SHARED_MEMORY_BYTES];
    };

    inline std::wstring BuildPipeName(std::uint32_t pid)
    {
        return std::wstring(VDTRACE_LOADER_MEMORY_PIPE_PREFIX) + std::to_wstring(pid);
    }

    inline std::wstring BuildSharedObjectName(const wchar_t* suffix, std::uint32_t clientPid, std::uint64_t sessionId)
    {
        return std::wstring(L"Local\\VDTraceLoaderMemory-")
            + suffix
            + L"-"
    ...
    inline void CopySharedName(wchar_t* destination, const std::wstring& source)
    {
        ...
        destination[0] = L'\0';
        wcsncpy_s(destination, VDTRACE_LOADER_SHARED_NAME_CHARS, source.c_str(), _TRUNCATE);
    }
        ...
        std::uint32_t payloadSize)
    {
        ...
        return VDTraceLoaderIpcTransport::WriteMessage(pipeHandle, header, payload, payloadSize);
    }

    inline bool ReadMessage(HANDLE pipeHandle, std::vector<std::uint8_t>& buffer)
    {
        return VDTraceLoaderIpcTransport::ReadMessage<MessageHeader>(
            pipeHandle,
            buffer,
            kMagic,
    ...
    {
        return VDTraceLoaderIpcTransport::ConnectToNamedPipe(BuildPipeName(pid), timeoutMs);
    }

    inline HANDLE CreatePipeServerInstance(std::uint32_t pid)
    {
        return VDTraceLoaderIpcTransport::CreateNamedPipeServer(BuildPipeName(pid));
    }
    ...
    {
        return VDTraceLoaderIpcTransport::WaitForPipeClient(pipeHandle);
    }

    inline void ClosePipe(HANDLE& pipeHandle)
    {
        VDTraceLoaderIpcTransport::ClosePipe(pipeHandle);
    }
}
