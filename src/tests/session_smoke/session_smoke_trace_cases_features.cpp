...
namespace session_smoke
{
    namespace
    {
        vdtrace::StepEvent BuildWriteAccountingEvent(uint64_t sequence)
        {
    ...
    void RunSessionSmokeTraceFeatureCases(const SessionSmokeSelection &selection, const SessionSmokeConfig &config)
    {
        ...
        if (ShouldRunCase(selection, "write-accounting"))
        {
            ...
            const uint64_t expected_events = 43000;
            {
                ...
                {
                    recorder.OnStep(BuildWriteAccountingEvent(index + 1));
                }
                ...
                Require(!recorder.IsWriting(), "write-accounting recorder did not drain");
