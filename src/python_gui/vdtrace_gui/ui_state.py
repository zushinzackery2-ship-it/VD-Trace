...
def create_ui_variables(root: tk.Misc, agent_path: Path) -> UiVariables:
    return UiVariables(
        ...
        session_status_var=tk.StringVar(master=root, value="可直接选择运行中的目标进程；Loader 只保留兼容入口。"),
        ...
        log_status_var=tk.StringVar(master=root, value="这里会显示目标发现、Loader 日志和控制命令结果。"),
