...
| `idle_escape_threshold` | `0` | 热旁路阈值；`0` 关闭，开启建议 `8` |
...
- **热循环旁路（`idle_escape_threshold`，默认关闭）**：设为 `8`（推荐）后，热循环回边重复到阈值次，DR 只在循环出口 + 帧返回下断，让循环原生自由跑，避免每圈都吃 VEH 异常。`0` 关闭；追踪病态热点可再调小，需要观察更多早期迭代可调大。
