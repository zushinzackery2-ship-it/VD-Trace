...
| `idle_escape_threshold` | `8` | 热旁路阈值，热点循环回边重复超过此次数后 DR 重编程到出口自由跑；`0` 关闭 |
...
- **热循环旁路（`idle_escape_threshold`，默认 8）**：热循环回边重复到阈值次后，DR 只在循环出口 + 帧返回下断，让循环原生自由跑，避免每圈都吃 VEH 异常。默认从 32 下调为 8：热身开销更小（尤其被反复重进的嵌套循环），而阈值高本来也换不来多少完整性（放行后本就是尽力而为）。追踪病态热点代码可再调小，`0` 关闭；需要观察更多早期迭代可调大。
