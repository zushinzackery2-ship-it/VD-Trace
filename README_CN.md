...
> 当前主线工作流由 Flutter GUI / C++ CLI / autostart + BepInEx plugin 组成，Loader Control IPC 负责目标进程中的 Agent 加载与会话发现。
...
| **运行时 PE 修正** | Agent 内置 Dump+Fix，修正节表偏移并清除无效 Security 目录，输出更适合 IDA/Ghidra 加载的 PE |
...
- **VEH 入口**：注册最高优先级 handler，分发 `EXCEPTION_SINGLE_STEP` 异常
- **无锁 Session 定位**：VEH 热路径通过原子指针定位活动会话，避免 mutex 阻塞
- **指令分类**：Zydis 反汇编引擎将指令映射为 Call / Jump / ConditionalJump / Return / Syscall / Interrupt 六类控制流节点
- **模块二分查找**：模块范围按基址排序后 O(log n) 定位
- **线程本地 region 缓存**：缓存 `VirtualQuery` 结果，减少热路径内核调用

### 双模后端

| 模式 | 触发条件 | DR 编程策略 |
|:-----|:---------|:------------|
| **DR (主力)** | 模块内正常追踪 | 条件跳转 → 两个目标边；直接 call/jmp → 目标边；间接跳转 → 尾部单步解析 |
| **TF (辅助)** | 匿名页 / 深度过滤 TF 规则 / 探针 step/write / 等待间接目标 | 设置 EFlags.TF，每条指令触发异常，退出区域后恢复 DR |
...
- **自动捕获**：枚举全部线程，在触发地址挂 DR，并持续刷新新线程
- **原子竞争**：首个命中线程晋升为 active tracing thread
- **block_main_thread**：主线程可加入已知线程但不挂断点，用于跳过主线程捕获
- **异步接力**：命中新线程创建 API 后切换追踪上下文
- **排队模式**：后续命中线程可排队，当前追踪结束后轮转处理

### IPC Agent 协议

| 字段 | 说明 |
|:-----|:-----|
| Agent 管道名 | `\\.\pipe\VDTrace-<PID>` |
| Loader 管道名 | `\\.\pipe\VDTraceLoaderControl` |
| 协议版本 | `kIpcVersion = 17` |
| 通信模式 | 请求-响应，消息模式 (`PIPE_TYPE_MESSAGE`) |
| 请求结构 | `IpcCommand`：version + type + payload union |
| 响应结构 | `IpcResponse`：version + status + message |
